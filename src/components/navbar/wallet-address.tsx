...
const WalletAddress = React.forwardRef<HTMLButtonElement, WalletAddressProps>(
  ({ walletAddress, walletType, className, ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={`${volkhov.className} dark:bg-gray-1000 flex h-12.5 w-45 cursor-pointer items-center justify-between
          rounded-[15px] border border-gray-500 bg-gray-100 pr-1.5 pl-3.5 ${className ?? ''}`}
        {...props}
      >
